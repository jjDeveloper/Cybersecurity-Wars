...
export default class BmSellScene extends Scene {
...
processInput(input: string) {
  const selectedItem = this.itemEngine.getBlackMarketItems().find(item => item.id === input);
  ...
  render() {
    ...
    .option-button:hover {
      ...
      <div class="menu-container">
        <p class="description">${scenesConfig.bmsell.description}</p>
        <ul class="options">
          ${this.itemEngine.getBlackMarketItems().map(
            (item) => html`
